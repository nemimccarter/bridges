...
{    
    ...
    // variables for bin solution
    double size = sqrt(density * n);
    int bpr = ceil(size / cutoff);
    int numbins = bpr * bpr;
    std::vector<particle_t*> *bins = new std::vector<particle_t*>[numbins];
	...
	dmin = 1.0;
        ...
        for( int particle_index = 0; particle_index < n; particle_index++ )
        {
            ...
            // find each particle bin num
            int cbin = binNum(particles[particle_index], bpr);

            // test if we need to compare with neighbor bins
            ...
            // loop through high and lower neighbors
            // apply force only between current particle
            // and all particles in the neighbor bins
